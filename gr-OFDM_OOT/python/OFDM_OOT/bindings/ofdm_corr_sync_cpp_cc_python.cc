/*
 * Copyright 2022 Free Software Foundation, Inc.
 *
 * This file is part of GNU Radio
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 */

/***********************************************************************************/
/* This file is automatically generated using bindtool and can be manually edited  */
/* The following lines can be configured to regenerate this file during cmake      */
/* If manual edits are made, the following tags should be modified accordingly.    */
// BINDTOOL_GEN_AUTOMATIC(0)
/* BINDTOOL_USE_PYGCCXML(0)                                                        */
/* BINDTOOL_HEADER_FILE(ofdm_corr_sync_cpp_cc.h)                                        */
/* BINDTOOL_HEADER_FILE_HASH(60a61f6cb19a3413e973ea4557bcb28b)                     */
/***********************************************************************************/

#include <pybind11/complex.h>
#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

namespace py = pybind11;

#include <gnuradio/OFDM_OOT/ofdm_corr_sync_cpp_cc.h>
// pydoc.h is automatically generated in the build directory
#include <ofdm_corr_sync_cpp_cc_pydoc.h>

void bind_ofdm_corr_sync_cpp_cc(py::module& m)
{

    using ofdm_corr_sync_cpp_cc = gr::OFDM_OOT::ofdm_corr_sync_cpp_cc;


    py::class_<ofdm_corr_sync_cpp_cc,
               gr::block,
               gr::basic_block,
               std::shared_ptr<ofdm_corr_sync_cpp_cc>>(
        m, "ofdm_corr_sync_cpp_cc", D(ofdm_corr_sync_cpp_cc))

        .def(py::init(&ofdm_corr_sync_cpp_cc::make),
             py::arg("nfft") = 1024,
             py::arg("n_guard") = 128,
             py::arg("corr_sz") = 64,
             D(ofdm_corr_sync_cpp_cc, make))


        ;
}
