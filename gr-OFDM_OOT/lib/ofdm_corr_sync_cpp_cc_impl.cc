      ...
      corr_sz(corr_sz)
{
    ...
    // nfft+guard for estimation; buf_sz - 1 previous input to form syncronized output
                                             ...
                                             gr_vector_void_star& output_items)
{
    ...
    for (int i = 0; i < processed_items; i++) {
        this->process_next(in + i + (corr_buf.size() - 1), out_corr + i, i);
    }
...
void ofdm_corr_sync_cpp_cc_impl::process_next(const gr_complex* in,
                                              gr_complex* out,
                                              int out_ind)
{
    ...
    *out = this->corr_buf[(corr_buf_st + out_rel_pos) % corr_buf.size()];
    ...
    if (no_sync_counter == 0) {
        ...
        if (avg_corr >= this->sync_corr_lvl) {
            // calculate weight average of freq offset and middle position
            float tot_weight = 0;
            float freq_offset = 0;
